...
| **Per-tab AI session resume** | After session restoration, claude / copilot / codex / codexpilot tabs reopen at *their specific* prior conversation (not just the most-recent) and carry permissive CLI flags from `[ai_resume]` in `alacritty.toml`. Resolved via per-PID metadata: `~/.claude/sessions/<pid>.json`, `~/.copilot/logs/process-<ts>-<pid>.log`, codex-family argv `resume <uuid>`, and — when argv carries no session — the codex rollout `~/.codex/sessions/<Y>/<M>/<D>/rollout-<ts>-<uuid>.jsonl` that the process currently holds **open** (read from its fd table via `PROC_PIDLISTFDS`; subagent rollouts are filtered out by `payload.source`, and remaining candidates rank by cwd match then mtime, with a per-save-tick claim set so sibling codex tabs don't collide). Reading the open fd replaced an earlier `pbi_start_tvsec`-proximity match, which resolved almost nothing: resuming a codex conversation appends to its **original** rollout, so the filename timestamp predates the process by days and failed the start-time window — leaving a multi-tab restore with at most one live codex session. Saved commands are normalized on load against current TOML flags, `copilot` restore defaults to `--mouse=on` unless you already configured a mouse flag, `codexpilot` sessions replay as `codexpilot resume <id>` instead of plain `codex`, and `codex resume --last` is not persisted or replayed for restored tabs because it collapses multiple tabs into one conversation. |
